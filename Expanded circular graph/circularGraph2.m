  ...
  methods
    function this = circularGraph2(adjacencyMatrix,varargin)
      ...
      for i = 1:length(adjacencyMatrix)
        ...
        this.Node(i).Color ='w'; %'k';% [0 0 1];%this.ColorMap(i,:);
        ...
        if row(i) ~= col(i)
          if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0 
            ...
            this.Node(row(i)).Connection(end+1) = line(...
              ...
              'Color', this.ColorMap(ceil(lineWidth2(i)*length(adjacencyMatrix)),:)); %this.ColorMap(row(i),:));%
              %'PickableParts','none');
      ...
      extent = max(extent(:));
      ax.XLim = ax.XLim + extent*[-1.2 1.2];
      fudgeFactor =1.2; %0.9;%1;% 0.5;%1;%5; %1.75; % Not sure why this is necessary. Eyeballed it.
